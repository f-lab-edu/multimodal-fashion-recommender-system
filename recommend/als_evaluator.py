...
class ALSEvaluator:
    ...
    - test jsonl로 Recall@K, HitRate@K, MAP@K 직접 계산
    """
    ...
    def __init__(self, model_dir: Path) -> None:
        ...
        self.train_user_items_csr: Optional[csr_matrix] = None
    ...
    def _build_user_items_csr_from_df(self, df: pd.DataFrame) -> csr_matrix:
        """
        df(user_id, item_id, ...) → users x items CSR (binary)
        """
        df = df[
        ...
        if df.empty:
            raise RuntimeError("주어진 데이터프레임에서 유효한 user/item이 없습니다.")
    ...
    def build_user_items_from_train(self, train_jsonl: Path) -> None:
        df_train = self._load_df(train_jsonl)
        self.train_user_items_csr = self._build_user_items_csr_from_df(df_train)

        logger.info(
            "[EVAL] train_user_items_csr shape: users=%s, items=%s",
            f"{self.train_user_items_csr.shape[0]:,}",
            f"{self.train_user_items_csr.shape[1]:,}",
        )

    def _build_test_targets(self, df_test: pd.DataFrame) -> Dict[int, Set[int]]:
        """
        test df → user_idx 별 정답 item_idx 집합
        """
        df = df_test[
    ...
    def _recommend_for_user(self, user_idx: int, k: int) -> np.ndarray:
        if self.train_user_items_csr is None:
            raise RuntimeError(
                "train_user_items_csr가 없습니다. build_user_items_from_train() 호출 필요."
            )

        user_row = self.train_user_items_csr[user_idx]
    ...
    def _eval_single_user(
        ...
        - total_pos: 정답 개수
        - hit_user_flag: 적어도 1개 맞췄으면 1
        - ap_user: AP@K
        - has_pos: 정답이 있는 유저인지 여부
        """
        if not pos_items:
    ...
    def evaluate_on_test(self, test_jsonl: Path, k: int = 10) -> Dict[str, float]:
        if self.train_user_items_csr is None:
            raise RuntimeError(
                "train_user_items_csr가 없습니다. "
                "build_user_items_from_train()을 먼저 호출해야 합니다."
            )

        df_test = self._load_df(test_jsonl)
        targets = self._build_test_targets(df_test)
        ...
        for user_idx, pos_items in targets.items():
            (
                hits_user,
                total_pos_user,
                hit_user_flag,
                ap_user,
                has_pos,
            ) = self._eval_single_user(user_idx, pos_items, k)
        ...
        if num_users_eval == 0 or total_positives == 0:
            recall_at_k = 0.0
            hit_rate = 0.0
        else:
            recall_at_k = hits / total_positives
            hit_rate = hit_users / num_users_eval
